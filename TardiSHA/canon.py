...
class FrequencyAnchor:
    ...
    def __complex__(self) -> complex:
        """Compatibility image only; exact identity remains the Decimal body above."""
...
def boundary_glyphs_from_digest(source_digest: str) -> tuple[str, str]:
    """Reject the superseded digest-endpoint selector.

    Final Equation Z requires the complete source emission: twelve finalized
    lanes and both contiguous Fraktur-Z cadence witnesses.  A 256-bit digest
    alone cannot regenerate that body and therefore cannot lawfully choose the
    Goetics.
    """
    raise ValueError(
