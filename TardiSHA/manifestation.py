...
from .aeon_layers import PHI, PHI_IMAGE
from .canon import (
    GLYPH_BODY,
    GLYPH_LAWS,
    QVector,
...
def parity_tangent(vector: Sequence[object]) -> tuple[int, int, int, int]:
    values = _tangent_tuple(vector)
    return (values[0], values[1], values[3], values[2])


def _sub(left: Sequence[object], right: Sequence[object]) -> tuple[int, int, int, int]:
    a, b = _q_tuple(left), _q_tuple(right)
    return tuple(x - y for x, y in zip(a, b))  # type: ignore[return-value]


def _neg(vector: Sequence[object]) -> tuple[int, int, int, int]:
    values = _tangent_tuple(vector)
    return tuple(-v for v in values)  # type: ignore[return-value]


def _add(left: Sequence[object], right: Sequence[object]) -> tuple[int, int, int, int]:
    a, b = _tangent_tuple(left), _tangent_tuple(right)
    return tuple(x + y for x, y in zip(a, b))  # type: ignore[return-value]


def endpoint_l1_projection(left: Sequence[object], right: Sequence[object]) -> int:
    """Legacy endpoint ranking projection. This is not terminal D-COMP."""
    a, b = _q_tuple(left), _q_tuple(right)
    return sum(abs(x - y) for x, y in zip(a, b))


_l1 = endpoint_l1_projection
...
def metric_norm_image(vector: Sequence[object]) -> float:
    """Display projection only.  Decisions use :func:`metric_square`."""
    values = _tangent_tuple(vector)
    return sqrt(sum(w * v * v for w, v in zip(Q_METRIC_WEIGHTS, values)))


# Compatibility name.  This value is never used as a decision gate in this module.
metric_norm = metric_norm_image
...
def _q_residue_norm(q: Sequence[object]) -> float:
    return sqrt(_q_residue_norm_square(q))
...
def ennead_saturate(row: int, shadow_debt: int | float | Fraction | Q5Fraction) -> EnneadResult:
    """Resolve Q2 pressure exactly, exposing legacy float images only as witnesses."""
...
def manifest(court: int, q_vector: QVector, *, differential_tension: float = 0.0) -> Manifestation:
    if differential_tension != 0.0:
        raise ValueError("differential_tension injection is not terminal D-COMP; supply both boundary Q-vectors")
    return close_boundary(q_vector, q_vector, court=court)
